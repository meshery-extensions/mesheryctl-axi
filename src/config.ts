...
export function resolveMesheryConfigPath(): string {
  const fromEnv = process.env['MESHERY_CONFIG']?.trim() || process.env['MESHERYCTL_CONFIG']?.trim();
...
export async function loadMesheryAuth(
  ...
  if (!current || !cfg.contexts?.[current]) {
    throw new AxiError(`No valid current-context in ${configPath}`, 'AUTH_REQUIRED', [
  ...
  if (!endpoint) {
    throw new AxiError(`Context ${current} has no endpoint in ${configPath}`, 'AUTH_REQUIRED');
  }
  ...
  const tokenPath = isAbsolute(location) ? location : join(homedir(), '.meshery', location);

  let tokenObj: MesheryTokenFile;
  try {
    tokenObj = JSON.parse(await readFile(tokenPath, 'utf8')) as MesheryTokenFile;
  } catch {
    throw new AxiError(`mesheryctl token file missing or invalid: ${tokenPath}`, 'AUTH_REQUIRED', [
  ...
  if (!tokenValue) {
    throw new AxiError(`Token file ${tokenPath} has no token field`, 'AUTH_REQUIRED', [
